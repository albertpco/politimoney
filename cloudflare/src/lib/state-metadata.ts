...
export const STATE_CODE_TO_NAME = new Map(
  US_STATES.map((state) => [state.code, state.name]),
);

export const STATE_NAME_TO_CODE = new Map(
  US_STATES.map((state) => [state.name, state.code]),
);

export const STATE_FIPS_TO_CODE = new Map(
  US_STATES.map((state) => [state.fips, state.code]),
);
