...
import {
  findHouseVoteByIdRepository,
} from "@/lib/data/vote-repository";

export const revalidate = 3600;

export async function generateMetadata({ params }: HouseVotePageProps): Promise<Metadata> {
  const { id } = await params;
  const vote = await findHouseVoteByIdRepository(id);
  if (!vote) return { title: "House vote not found | Politired" };
  const question = vote.voteQuestion ?? "Vote";
  return {
    title: `House Roll Call ${vote.rollCallNumber} | Politired`,
    description: `${question}${vote.result ? ` (${vote.result})` : ""}. View member breakdown, party split, and funding analysis.`,
  };
}

type HouseVotePageProps = {
...
function voteSummaryText(vote: { voteQuestion?: string; rollCallNumber: number }): string {
  return vote.voteQuestion ?? `House roll call ${vote.rollCallNumber}`;
}
...
export default async function HouseVotePage({ params }: HouseVotePageProps) {
  const { id } = await params;

  const [backend, runSummary, votes, bills] = await Promise.all([
    getDataBackendMode(),
    getLatestRunSummaryRepository(),
    findHouseVoteByIdRepository(id),
  ...
  const [analysis, memberVotes] = await Promise.all([
    analyzeHouseVoteFundingRepository({ voteId: vote.voteId }),
    getHouseVoteMemberVotesRepository(vote.voteId),
  ]);
  ...
  const subtitle = [
    `House roll call ${vote.rollCallNumber}`,
  ...
  return (
    <EntityDetailTemplate
      ...
      summary={
        <ClaimCard
          claim={`This House vote is linked to ${analysis?.groups.length ?? 0} funding groups and ${memberVotes.length} member vote records.`}
          level="high"
    ...
    >
      <section className="grid gap-3 md:grid-cols-3">
        ...
        <MetricCard
          label="Result"
          value={vote.result ?? "—"}
          delta={vote.voteType ?? "House vote"}
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="pt-muted text-sm">
                      {group.memberCount} members · {group.matchedCandidateCount} matched candidate records · ${Math.round(group.totalReceipts).toLocaleString()} linked receipts
                    </p>
                  </div>
      ...
      <SectionCard
        ...
        subtitle="House members tied to this roll call, with links into member profiles."
      >
        {memberVotes.length ? (
          <TableExplorer
